...
void preciceAdapter::Interface::readCouplingData(double relativeReadTime)
{
    ...
    for (uint i = 0; i < couplingDataReaders_.size(); i++)
    {
        ...
        for (std::size_t component = 0; component < nReadData; ++component)
        {
            maximumAbsoluteValue = std::max(maximumAbsoluteValue, std::abs(dataBuffer_[component]));
        }
        std::ostringstream receivedDetail;
        receivedDetail << std::setprecision(17) << maximumAbsoluteValue;
        adapterInfo("PHASE1K17_RECEIVED mesh=" + meshName_
                        + " data=" + couplingDataReader->dataName()
                        + " offset=" + std::to_string(relativeReadTime)
                        + " count=" + std::to_string(nReadData)
                        + " max_abs=" + receivedDetail.str(), "info");
