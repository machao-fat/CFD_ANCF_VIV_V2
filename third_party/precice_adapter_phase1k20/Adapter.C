...
namespace
{
std::size_t phase1k18RollbackIndex = 0;
std::size_t phase1k18CheckpointSaveIndex = 0;

void capturePhase1K18Snapshot(const fvMesh& mesh, const std::string& stage)
{
    const pointVectorField& point = mesh.lookupObject<pointVectorField>("pointDisplacement");
    const volVectorField& cell = mesh.lookupObject<volVectorField>("cellDisplacement");
    phase1k18diag::writeSnapshot(mesh, point, cell, stage);
}

void tracePhase1K17DisplacementState(
    const fvMesh& mesh,
    const std::string& cellName,
    const std::string& pointName,
    const pointField& checkpointPoints,
    const char* event)
{
    const label cylinder = mesh.boundaryMesh().findPatchID("cylinder");
    if (cylinder < 0 || !mesh.foundObject<volVectorField>(cellName)
        || !mesh.foundObject<pointVectorField>(pointName))
    {
        adapterInfo(std::string("PHASE1K17_CHECKPOINT event=") + event
                    + " fields_missing=1", "info");
        return;
    }
    const volVectorField& cell = mesh.lookupObject<volVectorField>(cellName);
    const pointVectorField& point = mesh.lookupObject<pointVectorField>(pointName);
    const vectorField& pointPatch = refCast<const vectorField>(
        point.boundaryField()[cylinder]);
    scalar cellMax = 0, pointMax = 0, meshDeltaMax = 0;
    forAll(cell.boundaryField()[cylinder], i)
    {
        cellMax = Foam::max(cellMax, mag(cell.boundaryField()[cylinder][i]));
    }
    forAll(pointPatch, i)
    {
        pointMax = Foam::max(pointMax, mag(pointPatch[i]));
    }
    if (checkpointPoints.size() == mesh.points().size())
    {
        forAll(mesh.points(), i)
        {
            meshDeltaMax = Foam::max(meshDeltaMax,
                                     mag(mesh.points()[i] - checkpointPoints[i]));
        }
    }
    std::ostringstream detail;
    detail << std::setprecision(17)
           << "PHASE1K17_CHECKPOINT event=" << event
           << " time=" << mesh.time().value()
           << " cell_max=" << cellMax
           << " point_max=" << pointMax
           << " mesh_delta_from_checkpoint=" << meshDeltaMax
           << " cell_old_times=" << cell.nOldTimes()
           << " point_old_times=" << point.nOldTimes();
    adapterInfo(detail.str(), "info");
}
}
...
void preciceAdapter::Adapter::configure()
{
    ...
    try
    {
        ...
        // The initial displacement is the start value of the first window.
...
void preciceAdapter::Adapter::execute()
{
    ...
    // Advance preCICE
    const double completedStep = timestepSolver_;
    advance();

    // Read checkpoint if required
    const bool retry = requiresReadingCheckpoint();
    const bool windowComplete = isCouplingTimeWindowComplete();
    if (retry)
    {
        ++phase1k18RollbackIndex;
        if (FSIenabled_)
        {
            tracePhase1K17DisplacementState(
                mesh_, FSI_->getCellDisplacementFieldName(),
                FSI_->getPointDisplacementFieldName(), meshPoints_,
                "before_rollback");
        }
        readCheckpoint();
    }
    ...
    // Retry: consume the endpoint produced by the preceding advance.
    // Accepted boundary: consume the next physical window's start value.
    // An unfinished substep retains its endpoint offset.
    if (isCouplingOngoing())
    {
        const double readOffset = (retry || !windowComplete) ? completedStep : 0.0;
        adapterInfo("PHASE1K16_READ_OFFSET retry=" + std::to_string(retry)
                        + " windowComplete=" + std::to_string(windowComplete)
                        + " offset=" + std::to_string(readOffset), "info");
        readCouplingData(readOffset);
        if (retry && readOffset == completedStep)
        {
            std::ostringstream stage;
            stage << "S2_after_readData_dt_retry_" << phase1k18RollbackIndex;
            capturePhase1K18Snapshot(mesh_, stage.str());
        }
    }
...
#define doLocalCode(GeomField)                                                                      \
    ...
    if (FSIenabled_)
    {
        adapterInfo("PHASE1K17_CHECKPOINT_DISCOVERY cell="
                    + std::to_string(mesh_.foundObject<volVectorField>(
                        FSI_->getCellDisplacementFieldName()))
                    + " point=" + std::to_string(mesh_.foundObject<pointVectorField>(
                        FSI_->getPointDisplacementFieldName())), "info");
    }
...
void preciceAdapter::Adapter::addCheckpointField(pointVectorField* field)
{
    if (field)
    {
        ...
        pointVectorSnapshots_.emplace_back();
        // TODO: Old time
...
void preciceAdapter::Adapter::readCheckpoint()
{
    ...
    {
        if (FSIenabled_ && pointVectorFields_.at(i)->name() == FSI_->getPointDisplacementFieldName())
        {
            pointVectorSnapshots_.at(i).restore(*pointVectorFields_.at(i));
            adapterInfo("PHASE1K20_RESTORE_VERIFY pointDisplacement checksum="
                        + pointVectorSnapshots_.at(i).checksumHex(), "info");
        }
        else
        {
        ...
        if (nOldTimes >= 1 && !(FSIenabled_ && pointVectorFields_.at(i)->name() == FSI_->getPointDisplacementFieldName()))
        {
            pointVectorFields_.at(i)->oldTime() == pointVectorFieldCopies_.at(i)->oldTime();
        }
        if (nOldTimes == 2 && !(FSIenabled_ && pointVectorFields_.at(i)->name() == FSI_->getPointDisplacementFieldName()))
        {
    ...
    {
        std::ostringstream stage;
        stage << "S1_after_rollback_restore_retry_" << phase1k18RollbackIndex;
        capturePhase1K18Snapshot(mesh_, stage.str());
        tracePhase1K17DisplacementState(
            mesh_, FSI_->getCellDisplacementFieldName(),
            FSI_->getPointDisplacementFieldName(), meshPoints_,
...
void preciceAdapter::Adapter::writeCheckpoint()
{
    ...
    for (uint i = 0; i < pointVectorFields_.size(); i++)
    {
        if (FSIenabled_ && pointVectorFields_.at(i)->name() == FSI_->getPointDisplacementFieldName())
        {
            pointVectorSnapshots_.at(i).capture(*pointVectorFields_.at(i));
            adapterInfo("PHASE1K20_SAVE_VERIFY pointDisplacement checksum="
                        + pointVectorSnapshots_.at(i).checksumHex(), "info");
        }
        else
        {
    ...
    {
        std::ostringstream stage;
        stage << "S0_after_checkpoint_save_" << ++phase1k18CheckpointSaveIndex;
        capturePhase1K18Snapshot(mesh_, stage.str());
        tracePhase1K17DisplacementState(
            mesh_, FSI_->getCellDisplacementFieldName(),
            FSI_->getPointDisplacementFieldName(), meshPoints_,
