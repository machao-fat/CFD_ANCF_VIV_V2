...
(
    ...
    :
    displacementMotionSolver(name, mesh, solverDict, typeName),
    ...
    newPoints(mesh.nPoints(), vector::zero),
    rbf(),
    ...
    twoDCorrector(mesh),
    nbPoints(0),
    faceCellCenters(coeffDict().lookupOrDefault("faceCellCenters", false)),
    cpu(false),
    corrector(false),
    k(0)
{
    if (faceCellCenters)
    {
        FatalErrorInFunction
            << "The OpenFOAM Foundation 10 port reads pointDisplacement and "
            << "requires faceCellCenters no" << exit(FatalError);
    }
...
tmp<pointField> RBFMeshMotionSolver::curPoints() const
{
    ...
    (
        new pointField(points0() + newPoints)
   );
    twoDCorrectPoints(tnewPoints.ref());
...
{
    // Experimental Phase 1K.13-B trace only.  Not present in the qualified RBF binary.
    static label diagMotionCallCounter = 0;
    const label diagCall = ++diagMotionCallCounter;
    Info << "K13B_TRACE stage=solve_begin"
         << " time=" << mesh().time().value()
         << " time_dir=" << mesh().time().timeName()
         << " call=" << diagCall
...

    {
        ...
        stage << "S3_before_correctBoundaryConditions_call_" << diagCall;
        const volVectorField& cell = mesh().lookupObject<volVectorField>("cellDisplacement");
        phase1k18diag::writeSnapshot(mesh(), pointDisplacement_, cell, stage.str());
    }
    ...
    {
        std::ostringstream stage;
        stage << "S4_after_correctBoundaryConditions_call_" << diagCall;
        const volVectorField& cell = mesh().lookupObject<volVectorField>("cellDisplacement");
        phase1k18diag::writeSnapshot(mesh(), pointDisplacement_, cell, stage.str());
    }
    forAll(movingPatchIDs, movingI)
    {
        const label patchI = movingPatchIDs[movingI];
        const valuePointPatchField<vector>& diagPatch =
            refCast<const valuePointPatchField<vector>>
            (
                pointDisplacement_.boundaryField()[patchI]
            );
        scalar diagMinNorm = GREAT;
        scalar diagMaxNorm = 0;
        forAll(diagPatch, pointI)
        {
            const scalar norm = mag(diagPatch[pointI]);
            diagMinNorm = min(diagMinNorm, norm);
            diagMaxNorm = max(diagMaxNorm, norm);
            Info << "K13B_TRACE stage=point_boundary_value"
                 << " time=" << mesh().time().value()
                 << " time_dir=" << mesh().time().timeName()
                 << " call=" << diagCall
                 << " rank=" << Pstream::myProcNo()
                 ...
                 << " point=" << pointI
                 << " value=" << diagPatch[pointI]
                 << " norm=" << norm << endl;
        }
        Info << "K13B_TRACE stage=point_boundary_summary"
             << " time=" << mesh().time().value()
             << " time_dir=" << mesh().time().timeName()
             << " call=" << diagCall
             << " rank=" << Pstream::myProcNo()
             << " pid=" << ::getpid()
             << " patch=" << mesh().boundaryMesh()[patchI].name()
             << " count=" << diagPatch.size()
             << " min_norm=" << (diagPatch.size() ? diagMinNorm : 0)
             << " max_norm=" << diagMaxNorm << endl;
    }
    forAll(motionCenters, patchI)
    {
        motionCenters[patchI].clear();
    }
    forAll(movingPatchIDs, movingI)
    {
        const label patchI = movingPatchIDs[movingI];
        const valuePointPatchField<vector>& displacementPatch =
            refCast<const valuePointPatchField<vector>>
            (
                pointDisplacement_.boundaryField()[patchI]
            );
        motionCenters[patchI].setSize(displacementPatch.size());
        forAll(motionCenters[patchI], pointI)
        {
            motionCenters[patchI][pointI] = displacementPatch[pointI];
        }
    }
    ...
    if (!rbf->rbf->computed)
    {
        ...
        const Foam::pointField & points = points0();
    ...
    // Coarsening may update its selected indices during interpolate().
    const int diagSelectedCount = rbf->selectedPositions.size();
    Info << "K13B_TRACE stage=control_summary"
         << " time=" << mesh().time().value()
         << " time_dir=" << mesh().time().timeName()
         << " call=" << diagCall
         << " rank=" << Pstream::myProcNo()
         << " pid=" << ::getpid()
         << " total=" << valuesField.size()
         << " moving=" << nbMovingFaceCenters
         << " static=" << nbStaticFaceCenters
         << " fixed=" << nbFixedFaceCenters
         << " selected=" << diagSelectedCount << endl;
    forAll(valuesField, controlI)
    {
        bool diagSelected = false;
        for (int selectedI = 0; selectedI < diagSelectedCount; ++selectedI)
        {
            if (rbf->selectedPositions(selectedI) == controlI)
            {
                diagSelected = true;
                break;
            }
        ...
        const char* category =
            static_cast<unsigned int>(controlI) < nbMovingFaceCenters ? "moving" :
            static_cast<unsigned int>(controlI) < nbMovingFaceCenters + nbStaticFaceCenters ? "static" :
            "fixed";
        Info << "K13B_TRACE stage=control_value"
             << " time=" << mesh().time().value()
             << " time_dir=" << mesh().time().timeName()
             << " call=" << diagCall
             << " rank=" << Pstream::myProcNo()
             ...
             << " value=" << valuesField[controlI]
             << " norm=" << mag(valuesField[controlI]) << endl;
    }
    ...
    scalar diagInterpolationMin = GREAT;
    scalar diagInterpolationMax = 0;
    forAll(valuesInterpolationField, pointI)
    {
        const scalar norm = mag(valuesInterpolationField[pointI]);
        diagInterpolationMin = min(diagInterpolationMin, norm);
        diagInterpolationMax = max(diagInterpolationMax, norm);
    }
    Info << "K13B_TRACE stage=interpolation_summary"
         << " time=" << mesh().time().value()
         << " time_dir=" << mesh().time().timeName()
         << " call=" << diagCall
         << " rank=" << Pstream::myProcNo()
         << " pid=" << ::getpid()
         << " points=" << valuesInterpolationField.size()
         << " min_norm=" << (valuesInterpolationField.size() ? diagInterpolationMin : 0)
         << " max_norm=" << diagInterpolationMax << endl;
    ...
    pointDisplacement_.primitiveFieldRef() = valuesInterpolationField;

    const tmp<pointField> diagCurrentTmp = curPoints();
    const pointField& diagCurrent = diagCurrentTmp();
    scalar diagMeshMax = 0;
    forAll(diagCurrent, pointI)
    {
        diagMeshMax = max(diagMeshMax, mag(diagCurrent[pointI] - points0()[pointI]));
    }
    Info << "K13B_TRACE stage=mesh_points_summary"
         << " time=" << mesh().time().value()
         << " time_dir=" << mesh().time().timeName()
         << " call=" << diagCall
         << " rank=" << Pstream::myProcNo()
         << " pid=" << ::getpid()
         << " points=" << diagCurrent.size()
         << " max_cur_minus_points0=" << diagMeshMax << endl;
    forAll(movingPatchIDs, movingI)
    {
        const label patchI = movingPatchIDs[movingI];
        const labelList& patchPoints = mesh().boundaryMesh()[patchI].meshPoints();
        scalar patchMax = 0;
        forAll(patchPoints, patchPointI)
        {
            ...
            (
                patchMax,
                mag(diagCurrent[patchPoints[patchPointI]] - points0()[patchPoints[patchPointI]])
            );
        }
        Info << "K13B_TRACE stage=cylinder_mesh_summary"
             << " time=" << mesh().time().value()
             << " time_dir=" << mesh().time().timeName()
             << " call=" << diagCall
             << " rank=" << Pstream::myProcNo()
             << " pid=" << ::getpid()
             << " patch=" << mesh().boundaryMesh()[patchI].name()
             << " count=" << patchPoints.size()
             << " max_cur_minus_points0=" << patchMax << endl;
        if (patchPoints.size())
        {
            const label first = patchPoints[0];
            const label last = patchPoints[patchPoints.size()-1];
            Info << "K13B_TRACE stage=cylinder_representative"
                 << " time=" << mesh().time().value()
                 << " time_dir=" << mesh().time().timeName()
                 << " call=" << diagCall
                 << " rank=" << Pstream::myProcNo()
                 << " pid=" << ::getpid()
                 << " first_index=" << first
                 << " first_displacement=" << diagCurrent[first] - points0()[first]
                 << " last_index=" << last
                 << " last_displacement=" << diagCurrent[last] - points0()[last] << endl;
        }
    }
    forAll(fixedPatchIDs, fixedI)
    {
        const label patchI = fixedPatchIDs[fixedI];
        const labelList& patchPoints = mesh().boundaryMesh()[patchI].meshPoints();
        if (patchPoints.size())
        {
            const label first = patchPoints[0];
            Info << "K13B_TRACE stage=far_field_representative"
                 << " time=" << mesh().time().value()
                 << " time_dir=" << mesh().time().timeName()
                 << " call=" << diagCall
                 << " rank=" << Pstream::myProcNo()
                 ...
                 << " patch=" << mesh().boundaryMesh()[patchI].name()
                 << " point_index=" << first
                 << " displacement=" << diagCurrent[first] - points0()[first] << endl;
        }
    }
    {
        std::ostringstream stage;
        stage << "S5_after_RBF_solve_call_" << diagCall;
        const volVectorField& cell = mesh().lookupObject<volVectorField>("cellDisplacement");
        const tmp<pointField> candidateTmp = curPoints();
        const pointField& candidate = candidateTmp();
        phase1k18diag::writeSnapshot(
            mesh(), pointDisplacement_, cell, stage.str(), &candidate);

        // Diagnostic-only hard stop: once the adapter has captured the first
        // retry read at relativeReadTime=dt, stop immediately after the next
        // RBF solve snapshot. This prevents a second physical window from
        // being accepted; no solver/coupling behavior is changed beforehand.
        const char* diagDir = std::getenv("PHASE1K18_DIAG_DIR");
        const char* stopAfterRetry = std::getenv("PHASE1K18_7B_STOP_AFTER_RETRY_S5");
        if (diagDir != nullptr && stopAfterRetry != nullptr
            && std::string(stopAfterRetry) == "1")
        {
            const std::string retryRead = std::string(diagDir)
                + "/S2_after_readData_dt_retry_1.json";
            if (::access(retryRead.c_str(), F_OK) == 0)
            {
                const std::string stopRecord = std::string(diagDir)
                    + "/controlled_stop_after_retry_S5.json";
                std::ofstream stopOut(stopRecord.c_str(), std::ios::out | std::ios::trunc);
                stopOut << "{\"classification\":\"CONTROLLED_STOP_AFTER_FIRST_RETRY_S5\","
                        << "\"stage\":";
                phase1k18diag::writeQuoted(stopOut, stage.str());
                stopOut << ",\"time_s\":" << mesh().time().value()
                        << ",\"call\":" << diagCall << "}\n";
                stopOut.close();
                Info << "PHASE1K18_7B_CONTROLLED_STOP after " << stage.str()
                     << "; first retry S0-S5 captured; exiting before another Fluid advance"
                     << endl;
                ::_exit(86);
            }
        }
