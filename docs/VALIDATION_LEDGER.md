...
| Phase 1K.28 old-native software reference | GENERIC_ALE_ROLLBACK_STATE_CAUSAL | On the authoritative old mesh with its own native 30.0 s fields and no `mapFields`, B0 attempts 3/4 had identical Fluid displacement (`d32bccc...b1fb339`) but raw Force/preWrite differences of `0.03743996584180332 N`; S0/S1 showed the `meshPhi`/`meshPhi_0` lifecycle mismatch. The isolated B1 G2 candidate made all 15 paired stages and raw pressure/viscous/total Force plus the complete preWrite payload exactly equal (`0 N`). Structure Force still differed because retry and accepted reads used `relativeReadTime=dt` and `0`. This authorizes the next CFD inner-convergence diagnostic only on the experimental B1 reference branch; it does not authorize an IQN retest, production repair, long-window run, or physical validation. |
