...
| Phase 1K.23 single-window convergence mechanism | NOT PASS / OPERATOR REPEATABILITY NOT ESTABLISHED | one window, 40-attempt diagnostic: 39 rollbacks and acceptance at the diagnostic limit; exact S0-to-S1 point/cell/mesh/ANCF restore checks pass, but same `D*` Force replay differs by `0.1057131957411209 N` versus the `1e-3 N` limit; inner-convergence A/B and repaired-adapter constant-relaxation baseline were not measured |
| Phase 1K.24 Fluid rollback/operator repeatability | FLUID_ROLLBACK_STATE_MISMATCH_LOCALIZED | Same-process S0/S1 instrumentation found post-advance old-time/objectRegistry/ALE lifecycle differences (`U/k/omega/Uf` histories, `U_0/k_0/omega_0/Uf_0`, `meshPhi`, `moving/changing`, and `V/V0/V00` proxies) before the second solve. Fresh-process A/B replay was repeatable (`||FB-FA||=0 N`) on the one-step accepted branch; same-process repeatability remains unestablished and inner-convergence A/B is not yet testable. |
| Phase 1K.25 minimal rollback repair | ROLLBACK_REPEATABILITY_NOT_RESTORED | Group 1 old-time/history canonicalization reduced `||F2-F1||` to `0.08047955765631062 N` but remained above the `1e-3 N` limit. Isolated Group 2 `meshPhi`, Group 3 `fvMesh`, and Group 5 ancillary function-object candidates did not restore repeatability. Fresh-process A/B controls were deterministic for each tested topology. Groups 4 and 6 remain NOT_MEASURED; CFD inner-convergence A/B and IQN retest are blocked. |
