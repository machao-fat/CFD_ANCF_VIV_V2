...
def _bounded_window_limit(bundle: HH06ContractBundle, requested: int | None) -> int:
    ...
    if bundle.case_id == "ANCF_SINGLE_SLICE_HIGHRE_0P2S_PREP_V1":
        ...
        if isinstance(authorized_windows, bool) or not isinstance(authorized_windows, int) or authorized_windows != 2:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.run and args.max_windows is None:
        parser.error("--max-windows is required with --run; HH06 requires exactly the authorized value 2")
    try:
