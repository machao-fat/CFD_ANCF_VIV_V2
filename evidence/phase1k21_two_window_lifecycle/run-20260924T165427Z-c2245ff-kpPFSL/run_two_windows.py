#!/usr/bin/python3.10
"""Single-use Phase 1K.21 two-window experimental checkpoint runner."""
...
def main():
    ...
    if run != RUN.resolve():
        raise SystemExit("run directory differs from the single authorized Phase 1K.21 attempt")
    case = run / "case"
    ...
    if json.loads((case / "contract.json").read_text())["execution_authorization"]["max_windows"] != 2:
        raise SystemExit("scratch execution authorization exceeds two windows")
