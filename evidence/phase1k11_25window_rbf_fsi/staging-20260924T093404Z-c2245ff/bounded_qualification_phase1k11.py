...
DEFAULT_CASE = Path(__file__).resolve().parent / "source_case"
...
EXPECTED_RBF_LIBRARY = Path(
    "/home/machao/OpenFOAM/machao-10/platforms/linux64GccDPInt32Opt/lib/libRBFMeshMotionSolver.so"
)
EXPECTED_RBF_SHA256 = "b27b347c4ba3026a495517ae7ae237ceb5cd8408ac0db9af6a58ccd83647a20c"
EXPECTED_RBF_BUILD_ID = "f5ba43436c00f75dd314136806e0a00c632d0d58"
EXPECTED_RBF_DICT_SHA256 = "0ffeeac3d86ccfce4f6468371c188d50f70342b77ea0e8bff61b0e6f36519f78"
...
EXPECTED_F0 = (0.0655270406896, 0.05872987414832, -2.10307854575e-21)
EXPECTED_F0_RESULT_SHA256 = "ce2cbf6edcda28ae46bf3d0a10b2a6d75ec8641b8417549818acda9c98369949"
EXPECTED_F0_REPORT_SHA256 = "4d50076b0c89175e9e6bcfbe9b7015f97e8d82a0a41b771552c18397533f0284"
EXPECTED_F0_PROVENANCE_SHA256 = "24d572f35e9b06bd34c3a934a67db61559e109e8f4d59a398075822cd208a7f2"
BINDING_EVIDENCE = REPO_ROOT / "evidence" / "phase1d6_python_precice_binding" / "qualification.json"
RUN_EVIDENCE_ROOT = REPO_ROOT / "evidence" / "phase1k11_25window_rbf_fsi"
...
def _verify_rbf_port(case_dir: Path) -> dict[str, str]:
    config = case_dir / "constant" / "dynamicMeshDict"
    config_sha = _verify_sha(config, EXPECTED_RBF_DICT_SHA256, "frozen TPS-RBF configuration")
    library_sha = _verify_sha(EXPECTED_RBF_LIBRARY, EXPECTED_RBF_SHA256, "verified OF10 RBF library")
    readelf = shutil.which("readelf")
    _require(readelf is not None, "readelf unavailable for RBF Build ID verification")
    notes = subprocess.run([readelf, "-n", str(EXPECTED_RBF_LIBRARY)],
                           capture_output=True, text=True, timeout=30, check=False)
    _require(notes.returncode == 0, "readelf failed on RBF library")
    match = re.search(r"Build ID:\s*(\S+)", notes.stdout)
    _require(match is not None and match.group(1) == EXPECTED_RBF_BUILD_ID,
             "RBF library Build ID differs from Phase 1K.9 qualification")
    return {"library_path": str(EXPECTED_RBF_LIBRARY.resolve()),
            "library_sha256": library_sha, "library_build_id": match.group(1),
            "dynamic_mesh_dict_sha256": config_sha}
...
def preflight(case_dir: Path, worker_arg: str, max_windows: int) -> tuple[LaunchPlan, dict[str, Any]]:
    ...
    _require(git_identity["tracked_tree_clean"], "Phase 1K.11 requires no tracked source/config modifications")
...
def _git_identity() -> dict[str, Any]:
    ...
    tracked = subprocess.run(["git", "diff", "--quiet", "HEAD", "--"], cwd=REPO_ROOT,
                             capture_output=True, text=True, timeout=10, check=False)
    _require(tracked.returncode in (0, 1), "could not inspect tracked source/config modifications")
    return {"branch": branch, "head": head, "worktree_clean": not bool(status.strip()),
            "tracked_tree_clean": tracked.returncode == 0,
...
def _read_new_trace_records(path: Path, offset: int, pending: str,
    ...
    for line in lines:
        ...
        _monitor_force_growth(record, state)
        count += 1
    return next_offset, new_pending, count


def _monitor_fluid_log(path: Path, offset: int, pending: str,
                       state: dict[str, Any]) -> tuple[int, str]:
    if not path.is_file():
    ...
    for line in lines:
        if "FOAM FATAL" in line or "Floating point exception" in line:
            raise LaunchContractError(f"OpenFOAM fatal diagnostic: {line.strip()}")
        if re.search(r"(?<![A-Za-z])(?:nan|[+-]?inf)(?![A-Za-z])", line, re.IGNORECASE):
            raise LaunchContractError(f"non-finite Fluid diagnostic: {line.strip()}")
        match = re.search(r"^(Mesh )?Courant Number mean:\s*\S+\s+max:\s*(\S+)", line)
        if match:
            label = "mesh_Co" if match.group(1) else "fluid_Co"
            value = float(match.group(2))
            _require(math.isfinite(value), f"non-finite {label}")
            state[label] = max(state.get(label, 0.0), value)
            _require(value <= 10.0, f"{label} hard stop: {value} > 10")
    return next_offset, new_pending


def _monitor_scalar_series(path: Path, state: dict[str, Any],
    ...
    with path.open("r", encoding="utf-8", errors="replace") as stream:
        for line in stream:
            ...
            if len(parts) < len(labels) + 1:
                continue  # incomplete record while OpenFOAM is writing
            try:
                values = [float(token) for token in parts[:len(labels) + 1]]
            except ValueError:
                continue
            _require(all(math.isfinite(value) for value in values),
                     f"non-finite postProcessing series: {path}")
            for label, value in zip(labels, values[1:]):
                state[label] = value
                if label == "minimum_cell_volume":
                    _require(value > 0, f"non-positive cell volume: {value}")
                if label == "max_omega":
                    _require(value < 1e8, f"omega runaway hard stop: max={value}")


def _monitor_force_growth(record: Mapping[str, Any], state: dict[str, Any]) -> None:
    raw = record.get("returned_force_raw_N")
    if not isinstance(raw, list) or len(raw) < 2:
        return
    value = math.hypot(float(raw[0]), float(raw[1]))
    _require(math.isfinite(value), "non-finite raw Force norm")
    previous = state.get("previous_force_norm")
    if previous is not None and value > 1.0 and value > 3.0 * previous:
        state["force_growth_count"] = state.get("force_growth_count", 0) + 1
    else:
        state["force_growth_count"] = 0
    _require(state["force_growth_count"] < 2,
             f"two successive >3x raw Force increases above 1 N; latest={value}")
    state["previous_force_norm"] = value


def _monitor_saved_cell_volumes(case_dir: Path, state: dict[str, Any]) -> None:
    checked: set[str] = state.setdefault("checked_volume_times", set())
    for directory in case_dir.iterdir():
        if not directory.is_dir() or directory.name in checked:
            continue
        try:
            time_value = float(directory.name)
        except ValueError:
            continue
        if time_value <= EXPECTED_RELEASE_TIME:
        ...
        if match is None:
            continue  # field still being written
        count = int(match.group(1))
        _require(count == 46826, f"unexpected saved cell-volume count at {directory.name}: {count}")
        start = match.end()
        end = start + count * 8
        if len(data) <= end or data[end:end + 1] != b")":
            continue  # not a complete binary field yet
        minimum = min(value[0] for value in struct.iter_unpack("<d", data[start:end]))
        _require(math.isfinite(minimum) and minimum > 0,
                 f"non-positive/non-finite saved cell volume at {directory.name}: {minimum}")
        state["minimum_cell_volume"] = min(state.get("minimum_cell_volume", minimum), minimum)
        checked.add(directory.name)
...
def _process_snapshot(pid: int | None) -> dict[str, Any] | None:
    ...
    libraries = sorted({
        ...
        and ("libprecice.so" in line or "libpreciceAdapterFunctionObject.so" in line
             or "libRBFMeshMotionSolver.so" in line)
    })
...
def run_bounded(plan: LaunchPlan, *, preflight_report: Mapping[str, Any] | None = None,
    ...
    _require(git_identity["tracked_tree_clean"], "tracked source/config must be clean before Phase 1K.11 runtime")
    ...
    rbf_port = _verify_rbf_port(plan.case_dir)
    ...
    fluid_offset = 0
    fluid_pending = ""
    fluid_monitor_state: dict[str, Any] = {}
    ...
    try:
        ...
        with lock_path.open("a+", encoding="utf-8") as lock_stream:
            ...
            while any(process.poll() is None for process in processes.values()):
                try:
                    ...
                    fluid_offset, fluid_pending = _monitor_fluid_log(
                        run_dir / "fluid.stdout", fluid_offset, fluid_pending, fluid_monitor_state)
                    for object_name, labels in (
                        ...
                        _monitor_scalar_series(actual_plan.case_dir / "postProcessing" / object_name
                                               / "30" / "volFieldValue.dat", fluid_monitor_state, labels)
                    _monitor_saved_cell_volumes(actual_plan.case_dir, fluid_monitor_state)
                    ...
                    if fluid_snapshot:
                        ...
                        rbf_paths = [Path(value).resolve() for value in fluid_snapshot["coupling_libraries"]
                                     if "libRBFMeshMotionSolver.so" in value]
                        if rbf_paths:
                            _require(rbf_paths == [EXPECTED_RBF_LIBRARY.resolve()],
                                     f"loaded RBF library path mismatch: {rbf_paths}")
                            _verify_sha(rbf_paths[0], EXPECTED_RBF_SHA256, "loaded RBF library")
                    if failure_text:
                        break
                except Exception as exc:
        ...
        rbf_paths = [Path(value).resolve() for value in (fluid_snapshot or {}).get("coupling_libraries", [])
                     if "libRBFMeshMotionSolver.so" in value]
        if rbf_paths != [EXPECTED_RBF_LIBRARY.resolve()]:
            failure_text = failure_text or f"actual loaded RBF library was not confirmed: {rbf_paths}"
    ...
    summary = {
        "classification": "PHASE1K11_RUNTIME_COMPLETED" if failure_text is None else "PHASE1K11_RUNTIME_STOPPED_OR_FAILED",
        ...
        "fluid_live_monitor": {key: sorted(value) if isinstance(value, set) else value
                               for key, value in fluid_monitor_state.items()},
    ...
    if failure_text is not None:
        raise LaunchContractError(f"Phase 1K.11 stopped without retry: {failure_text}; evidence={run_dir}")
    return summary
