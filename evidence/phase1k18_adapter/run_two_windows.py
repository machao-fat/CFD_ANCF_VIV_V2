#!/usr/bin/python3.10
"""Single-use Phase 1K.18 two-window lifecycle qualification runner."""
...
RUN = ROOT / "evidence/phase1k18_adapter/run-20260924T121817Z-c2245ff"
...
REFERENCE_XML = ROOT / "evidence/phase1k17_adapter/run-20260924T120749Z-c2245ff/case/precice-config.xml"
...
def main():
    ...
    if run != RUN.resolve():
        raise SystemExit("run directory differs from the single authorized Phase 1K.18 attempt")
    case = run / "case"
    marker = run / "launch_started.json"
    if marker.exists():
        raise SystemExit("two-window run already launched; refusing a second attempt")
    if not case.is_dir():
    ...
    if len(caps) != 1 or caps[0].get("value") != "2":
        raise SystemExit("preCICE physical-window bound is not exactly two")
    ...
    expected_xml = REFERENCE_XML.read_text()
    normalized_xml = xml.replace(str(socket_dir), "RUN_SOCKET")
    normalized_xml = normalized_xml.replace('max-time-windows value="2"',
                                            'max-time-windows value="1"')
    expected_normalized = expected_xml.replace(
        "/home/machao/projects/CFD_ANCF_VIV_V2/evidence/phase1k17_adapter/run-20260924T120749Z-c2245ff/precice-sockets",
        "RUN_SOCKET")
    if normalized_xml.replace("Phase 1K.18 lifecycle diagnostic: exactly two physical windows.",
                              "Phase 1K.17 experimental adapter diagnostic: exactly one window.") != expected_normalized:
        raise SystemExit("scratch XML differs from the accepted Phase 1K.17 coupling config beyond the two-window/socket edits")
