...
def preflight() -> dict:
    ...
    if auth["max_windows"] != 1 or coupling["accepted_window_limit"] != 1:
        raise RuntimeError("scratch contract is not hard-limited to one window")
    if coupling["max_iterations"] != 2 or coupling["min_iterations"] != 2:
        raise RuntimeError("scratch cap must allow exactly two attempts")
    if coupling["dt_s"] != 0.0002 or coupling["duration_s"] != 0.0002:
        raise RuntimeError("scratch physical timestep/window mismatch")
...
def execute(identity: dict) -> dict:
    ...
    marker = {"started_unix": time.time(), "run_dir": str(RUN),
              ...
              "structure_command": ["/usr/bin/python3.10", str(PARTICIPANT),
                  ...
                  "--max-windows", "1", "--audit-output", str(audit), "--trace-output", str(trace)],
    ...
    structure_command = ["/usr/bin/python3.10", str(PARTICIPANT), "--case", str(CASE),
                         "--run", "--worker", str(WORKER), "--max-windows", "1",
