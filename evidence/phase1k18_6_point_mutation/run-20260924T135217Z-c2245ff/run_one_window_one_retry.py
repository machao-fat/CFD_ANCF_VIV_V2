        ...
        while any(proc.poll() is None for proc in processes.values()):
            ...
            for log_path in (fluid_out, fluid_err, structure_out, structure_err):
                if log_path.exists():
                    ...
                    # Keep this case-sensitive: OpenFOAM prints the benign startup banner
                    # "sigFpe : Enabling ..."; only uppercase SIGFPE denotes a failure marker.
                    fatal_marker = re.search(
                        r"FOAM FATAL|SIGFPE|Floating point exception", tail
                    )
