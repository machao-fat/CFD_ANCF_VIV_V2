...
ADAPTER = Path("/home/machao/projects/CFD_ANCF_VIV_V2/evidence/phase1k18_6_point_mutation/run-20260924T135724Z-c2245ff/adapter-lib/libpreciceAdapterPhase1K18Diag.so")
RBF = Path("/home/machao/projects/CFD_ANCF_VIV_V2/evidence/phase1k18_6_point_mutation/run-20260924T135724Z-c2245ff/rbf-lib/libRBFMeshMotionSolverPhase1K18Diag.so")
...
def preflight() -> dict:
    ...
    previously_used_empty_socket = socket_dir == Path(
        "/home/machao/projects/CFD_ANCF_VIV_V2/evidence/phase1k18_6_point_mutation/run-20260924T135724Z-c2245ff/precice-sockets"
    ).resolve()
    if not socket_dir.is_relative_to(RUN.resolve()) and not previously_used_empty_socket:
...
def execute(identity: dict) -> dict:
    ...
    fluid_script = ("set +u; source /opt/openfoam10/etc/bashrc >/dev/null; set -u; "
                    "cd \"$1\" || exit 91; "
                    "case_pwd=$(pwd -P); case_expected=$(realpath \"$1\"); "
                    "printf 'PHASE1K18_FLUID_PWD=%s\\n' \"$case_pwd\"; "
                    "test \"$case_pwd\" = \"$case_expected\" || exit 92; "
        ...
        while any(proc.poll() is None for proc in processes.values()):
            ...
            for log_path in (fluid_out, fluid_err, structure_out, structure_err):
                if log_path.exists():
                    ...
                    # Exclude FOAM_SIGFPE: it is a benign startup environment token.
                    fatal_marker = re.search(
                        r"FOAM FATAL|(?<![A-Za-z0-9_])SIGFPE\b|Floating point exception", tail
                    )
                    nonfinite_marker = re.search(r"\b(?:nan|inf)\b", tail, re.IGNORECASE)
                    if fatal_marker or nonfinite_marker:
